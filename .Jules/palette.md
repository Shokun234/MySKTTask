...
## 2025-05-27 - Centralized Theme Management and Switch Accessibility
**Learning:** Using an ARIA `switch` role for theme toggles requires manual synchronization of `aria-checked` and descriptive `aria-label` (e.g., 'Switch to Dark Mode') to remain accessible. Centralizing this logic into a dedicated UI update function ensures consistency across initial load and user interactions.
**Action:** Always use `role="switch"` for theme toggles and pair it with a centralized `updateThemeUI` function that manages body classes, ARIA attributes, and the browser's `theme-color` meta tag.
