...
function renderDashboard() {
  // Bolt: Optimized dashboard by consolidating multiple filter/length calls into a single pass.
  // Also calculates per-subject progress in the same loop to avoid O(Subjects * N) complexity.
  // Reduces total iterations from (3 + 2 + 2*N) to 1.
...
function progressHtml(stats) {
  ...
  return subjects.map(subject => {
    const { total, done } = stats[subject];
